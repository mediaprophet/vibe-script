...
`vibe-0.1` is the current dialect. Grammar **may grow** when humans need a better form (versioned, documented). The catalog of named work also grows (core §11.5). Do not invent a private dialect or families in this repo. Do not treat `vibe-host-0.1` as a freeze.
...
| A7 | Catalog grow (Wave 40 / `7928e3d`) | **Landed** — named work ~905 → **1121** including `LinearAlgebra.*`. Count the tip. |
| A8 | `GraphDatabase.volume_open` / `volume_commit` | Landed (B-001). Native sanctuary; wasm E300. |
| A9 | Diagnose JSON `errors[]` | Landed |
| A10 | Public relay / datatracker | **Planned**. No invented ticks. |
...
| E6 | **Agent helper** | Landed | `skills/SKILL.md`: human-alone, no invent ids, live / planned. |
...
| E10 | **Refresh bundled `docs/pkg/vibe` WASM** | Landed | Rebuilt from QualiaDB tag `v0.0.38` `crates/vibe-wasm` (web target). Package stamp `0.0.38`. This page evals `LinearAlgebra.dot` and two-segment logic families. Residual: `honesty: "local"` descriptor (not the numeric solver); `TemporalAndDescriptionLogic.ltl.*` three-segment dotted path E100. |
| E11 | **Hot-edit in this playground** | Remaining (north star) | Write a script, run it, change it without rebuilding the app. Browser WASM eval is local; claiming live Poet furniture here is planned. |
